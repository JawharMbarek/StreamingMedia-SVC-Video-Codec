...
%http://media.xiph.org/ldv/pub/test_sequences/720p/
...
\begin{figure}[htb]
	...
	\caption[Referenzencoder]{Erzeugung von 360p und 180p YUV-Videodateien}
...
\begin{figure}[htb]
	...
	\caption[Referenzencoder]{Dekodierung und Rekonstruktion des Videostroms}
...
\begin{lstlisting}[label=Encoder,caption=Aufruf BitStreamExtractorStaticd ,captionpos=b,breaklines=true,mathescape=true]
$\textbf{BitStreamExtractorStaticd}$ InputDatei.svc OutputDatei.sv –sl 5
\end{lstlisting}

Als Resultät erhält man eine neue valide H264-SVC-kodierte Videodatei, die nur die Layer 0 bis 5 enthält.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.9\textwidth]{bilder/encoder/bitstreamdebug.jpg}
	\caption[Referenzencoder]{Anzeige der Layerstruktur mit Hilfe von BitStreamExtractorStaticd}
	\label{fig:bitstreamdebu}
	\includegraphics[width=0.9\textwidth]{bilder/encoder/layerextrahiert.jpg}
	\caption[Referenzencoder]{Anzeige der neuen Layerstruktur mit Hilfe von BitStreamExtractorStaticd}
	\label{fig:layerextrahiert}
\end{figure}

\subsection{Evaluation}
Der JSVM-Referenzcode ist ein sehr mächtiges Tool und derzeit das einzige frei verfügbare Programm, um
...
Die Arbeit mit dem Codec ist über die Kommandozeile und Eingabeparameter nicht sehr konfortable, 
vereinfacht aber dafür die Einbindung in andere Programmen. Die mitgelieferte Dokumentation und
Beispielprojekte machen es relativ zügig möglich selber mit dem Codec zu experimentieren. Allerdings
gibt der Codec selbst nur wenig Hinweis, warum manche Settings nicht funktionieren   

Das große Problem des JSVM-Codecs sind die Geschwindigkeit der Enkodierung und die möglichen 
Ausgangsformate. Diese beiden Aspekte machen jegliche Verwendung in Realanwendungen unmöglich.
Der Codec läuft nur auf einem Prozessorkern und nutzt dadurch die Möglichkeiten modernern PCs
nicht. Dadurch ist die Enkodierung sehr langsam. 
Bei regulärem CIF und QCIF Videos bentrug die Geschwidgkeit ca. 5 bis 6 Sekunden pro Frame. Bei 
Testreihen mit HD-Material dauerte die Enkodierung von 100 Frames mehrere Stunden. 
Zusätzlich unterstützt der Codec derzeit als Eingabeformat nur YUV-Video wodurch eine sinnvolle 
...
H264-SVC bekannt zu machen und diesen eine technische Basis zu geben. 
...
MainConcept bietet einen H264-SVC-Decoder innerhalb ihres kostenpflichtigen MainConcept SDKs an. In diesem 
...
Der SVC-Decoder bietet Möglichkeit sowohl alle Layer gleichzeitig anzuzeigen als auch selektiv nur
einen bestimmten Layer anzuzeigen, z.B. nur Layer 0 mit der Framerate 7.5 Frames.  

\begin{figure}[htb]
	\centering
	\includegraphics[width=1\textwidth]{bilder/mainConcept.jpg}
	\caption[Referenzencoder]{MainConcept SVC-Dekoder}
	\label{fig:mainConcept}
\end{figure}

\subsection{Open SVC Decoder}
http://sourceforge.net/apps/mediawiki/opensvcdecoder/

\subsection{SVC Stream Player}
http://svcp2pstreampla.sourceforge.net/
